...
export default function CatalogPage() {
  ...
  async function deleteSection(sectionId: string) {
    ...
    if (!confirmed) return
    const { error: delError } = await supabase.from('catalog_sections').delete().eq('id', sectionId)
  ...
  async function saveItemEdits(itemId: string) {
    ...
    if (error) { console.error(error); return }
    setItems((prev) => prev.map((i) => i.id === itemId ? { ...i, name: edits.name, description: edits.description, unit: edits.unit } : i))
  ...
  async function deleteItem(itemId: string) {
    ...
    if (!confirmed) return
    const { data: costRowData, error: fetchErr } = await supabase.from('catalog_cost_rows').select('id').eq('item_id', itemId)
    if (fetchErr) { console.error(fetchErr); return }
    const costRowIds = (costRowData || []).map((r: any) => r.id)
    if (costRowIds.length > 0) {
      const { error: tirErr } = await supabase.from('template_item_rows').delete().in('catalog_cost_row_id', costRowIds)
      if (tirErr) { console.error(tirErr); return }
    }
    const { error: rowsError } = await supabase.from('catalog_cost_rows').delete().eq('item_id', itemId)
    if (rowsError) { console.error(rowsError); return }
    const { error: itemError } = await supabase.from('catalog_items').delete().eq('id', itemId)
  ...
  async function moveItem(itemId: string, newSectionId: string) {
    ...
    const { error } = await supabase.from('catalog_items').update({ section_id: newSectionId }).eq('id', itemId)
    if (error) { console.error(error); return }
  ...
  async function addCostRow(itemId: string) {
    ...
    const { error } = await supabase.from('catalog_cost_rows').insert([{ item_id: itemId, name: newRowName.trim(), unit: newRowUnit.trim(), unit_cost: Number(newRowUnitCost) }])
    if (error) { console.error(error); return }
  ...
  async function saveCostRowEdits(rowId: string, itemId: string) {
    ...
    if (!edits) return
    const { error } = await supabase.from('catalog_cost_rows').update({ name: edits.name, unit: edits.unit, unit_cost: Number(edits.unit_cost) || 0 }).eq('id', rowId)
    if (error) { console.error(error); return }
  ...
  async function deleteCostRow(rowId: string, itemId: string) {
    ...
    if (!confirmed) return
    const { error: tirErr } = await supabase.from('template_item_rows').delete().eq('catalog_cost_row_id', rowId)
    if (tirErr) { console.error(tirErr); return }
    const { error } = await supabase.from('catalog_cost_rows').delete().eq('id', rowId)
    if (error) { console.error(error); return }
    setRowEdits((prev) => { const next = { ...prev }; delete next[rowId]; return next })
    getCostRows(itemId)
  }
    ...
    <div className="flex h-screen bg-[#f4f7fb]">
      ...
      <aside className="w-60 bg-white border-r border-gray-100 p-6 shadow-sm flex-shrink-0">
        ...
        <nav className="flex flex-col gap-3 text-sm">
          ...
          <Link href="/catalog" className="text-blue-700 font-medium">Catalog</Link>
      ...
      <div className="w-64 bg-white border-r p-6 flex flex-col flex-shrink-0">
        ...
        <div className="mb-4">
          ...
          />
          <button onClick={addSection} className="w-full bg-blue-600 text-white py-2 rounded text-sm">+ Add Section</button>
        </div>
          ...
          {sections.map((section) => (
            <div key={section.id}>
              {editingSectionId === section.id ? (
                ...
                <div
                  className={`group flex items-center justify-between px-3 py-2 rounded-lg cursor-pointer text-sm ${selectedSectionId === section.id ? 'bg-blue-100 text-blue-700 font-medium' : 'hover:bg-gray-100 text-gray-700'}`}
                ...
                >
                  ...
                  <div className="flex gap-1 ml-2 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button onClick={(e) => { e.stopPropagation(); setEditingSectionId(section.id); setEditingSectionName(section.name) }} className="text-xs text-gray-400 hover:text-blue-600 px-1" title="Rename">✎</button>
                    <button onClick={(e) => { e.stopPropagation(); deleteSection(section.id) }} className="text-xs text-gray-400 hover:text-red-500 px-1" title="Delete">✕</button>
                  </div>
                </div>
              )}
            </div>
          ...
          <>
            <div className="bg-white p-4 rounded-xl border border-gray-200 mb-6 shadow-sm">
              ...
              <div className="flex gap-3">
                <input className="flex-1 p-2 border rounded text-sm" placeholder="Item name" value={newItemName} onChange={(e) => setNewItemName(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && addItem()} />
                <button onClick={addItem} className="bg-green-600 text-white px-4 py-2 rounded text-sm whitespace-nowrap">+ Add Item</button>
              </div>
            </div>
                    ...
                    <div key={item.id} className="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
                      <div className="flex items-center justify-between px-5 py-4">
                        ...
                        <div className="flex items-center gap-4 ml-4">
                          ...
                          <button onClick={() => deleteItem(item.id)} className="text-xs text-red-400 hover:text-red-600 px-1">Delete</button>
                          <span className="text-gray-400 text-sm cursor-pointer select-none" onClick={() => toggleItem(item)}>{isExpanded ? '▲' : '▼'}</span>
                        </div>
                      </div>
                      ...
                      {isExpanded && (
                        <div className="border-t border-gray-100 px-5 py-4">
                          ...
                          <div className="mb-5">
                            ...
                            <div className="grid grid-cols-3 gap-3 mb-2">
                              <div>
                                ...
                                <input className="w-full p-2 border rounded text-sm focus:outline-none focus:ring-2 focus:ring-blue-300" value={edits?.name ?? item.name ?? ''} onChange={(e) => updateItemEdit(item.id, 'name', e.target.value)} />
                              </div>
                              <div>
                                <label className="text-xs text-gray-400 block mb-1">Unit</label>
                                <input className="w-full p-2 border rounded text-sm focus:outline-none focus:ring-2 focus:ring-blue-300" placeholder="e.g. sqft, ea, hrs" value={edits?.unit ?? item.unit ?? ''} onChange={(e) => updateItemEdit(item.id, 'unit', e.target.value)} />
                              </div>
                              <div className="flex items-end">
                                <button onClick={() => saveItemEdits(item.id)} disabled={!hasUnsavedEdits} className={`w-full px-4 py-2 rounded text-sm font-medium transition-colors ${hasUnsavedEdits ? 'bg-blue-600 text-white hover:bg-blue-700' : 'bg-gray-100 text-gray-400 cursor-not-allowed'}`}>Save changes</button>
                              </div>
                            </div>
                            <div>
                              <label className="text-xs text-gray-400 block mb-1">Description (customer-facing)</label>
                              <textarea className="w-full p-2 border rounded text-sm focus:outline-none focus:ring-2 focus:ring-blue-300 resize-none" rows={4} placeholder="This description will appear on the customer proposal..." value={edits?.description ?? item.description ?? ''} onChange={(e) => updateItemEdit(item.id, 'description', e.target.value)} />
                            </div>
                          </div>
                          ...
                          <div className="mb-4 flex items-center gap-3">
                            ...
                            <select className="border rounded-lg px-3 py-1.5 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-blue-300" defaultValue="" onChange={(e) => { if (e.target.value) moveItem(item.id, e.target.value) }}>
                          ...
                          <div className="space-y-2 mb-4">
                            {rows.length === 0 && <p className="text-xs text-gray-400 mb-3">No cost rows yet. Add one below.</p>}
                            {rows.map((row) => {
                              ...
                              return (
                                <div key={row.id} className="grid grid-cols-5 gap-3 items-center bg-gray-50 rounded-lg px-3 py-2">
                                  <input className="w-full p-1.5 border rounded text-sm bg-white focus:outline-none focus:ring-2 focus:ring-blue-300" value={rEdits?.name ?? row.name ?? ''} onChange={(e) => updateRowEdit(row.id, 'name', e.target.value, row)} />
                                  <input className="w-full p-1.5 border rounded text-sm bg-white focus:outline-none focus:ring-2 focus:ring-blue-300" value={rEdits?.unit ?? row.unit ?? ''} placeholder="unit" onChange={(e) => updateRowEdit(row.id, 'unit', e.target.value, row)} />
                                  <input type="number" className="w-full p-1.5 border rounded text-sm bg-white focus:outline-none focus:ring-2 focus:ring-blue-300" value={rEdits?.unit_cost ?? String(row.unit_cost ?? '0')} onChange={(e) => updateRowEdit(row.id, 'unit_cost', e.target.value, row)} />
                                  <button onClick={() => saveCostRowEdits(row.id, item.id)} disabled={!hasRowEdits} className={`px-3 py-1.5 rounded text-xs font-medium transition-colors ${hasRowEdits ? 'bg-blue-600 text-white hover:bg-blue-700' : 'bg-gray-100 text-gray-400 cursor-not-allowed'}`}>Save</button>
                                  <button onClick={() => deleteCostRow(row.id, item.id)} className="text-xs text-red-400 hover:text-red-600 text-right">Delete</button>
                                </div>
                              )
                            })}
                          </div>
                          ...
                          <div className="border-t border-dashed border-gray-200 pt-4">
                            ...
                            <div className="grid grid-cols-4 gap-3 items-end">
                              <div>
                                <label className="text-xs text-gray-400 block mb-1">Name</label>
                                <input className="w-full p-2 border rounded text-sm" placeholder="e.g. Labor" value={newRowName} onChange={(e) => setNewRowName(e.target.value)} />
                              </div>
                              <div>
                                <label className="text-xs text-gray-400 block mb-1">Unit</label>
                                <input className="w-full p-2 border rounded text-sm" placeholder="e.g. hrs" value={newRowUnit} onChange={(e) => setNewRowUnit(e.target.value)} />
                              </div>
                              <div>
                                <label className="text-xs text-gray-400 block mb-1">Unit cost ($)</label>
                                <input type="number" className="w-full p-2 border rounded text-sm" placeholder="0.00" value={newRowUnitCost} onChange={(e) => setNewRowUnitCost(e.target.value)} />
                              </div>
                              <button onClick={() => addCostRow(item.id)} className="bg-blue-600 text-white px-4 py-2 rounded text-sm h-[38px]">+ Add Row</button>
                            </div>
                          </div>
