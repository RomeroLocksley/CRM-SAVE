    ...
    <div className="flex h-screen overflow-hidden" style={{ background: '#f4f7fb' }}>
      ...
      <div className="flex flex-col flex-shrink-0 overflow-hidden" style={{ width: 220, background: 'white', borderRight: '0.5px solid #eee' }}>
        <div style={{ padding: '20px 16px 12px', borderBottom: '0.5px solid #f0f0f0' }}>
          ...
          <input placeholder="Template name" value={newTemplateName} onChange={(e) => setNewTemplateName(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && createTemplate()}
            style={{ ...inputStyle, width: '100%', boxSizing: 'border-box', marginBottom: 8 }} />
          <input placeholder="Description (optional)" value={newTemplateDescription} onChange={(e) => setNewTemplateDescription(e.target.value)}
            style={{ ...inputStyle, width: '100%', boxSizing: 'border-box', marginBottom: 8 }} />
          <button onClick={createTemplate} style={{ width: '100%', padding: '8px', borderRadius: 8, background: '#185FA5', color: 'white', fontSize: 13, fontWeight: 500, border: 'none', cursor: 'pointer' }}>
            + New Template
          </button>
        </div>
        <div style={{ flex: 1, overflowY: 'auto', padding: '8px' }}>
          {templates.map((t) => (
            <div key={t.id}
              ...
              className="group"
              style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '9px 10px', borderRadius: 8, cursor: 'pointer', marginBottom: 2, background: selectedTemplateId === t.id ? '#E6F1FB' : 'transparent' }}
            ...
            <>
              ...
              {!showAddSection ? (
                ...
                <div style={{ background: 'white', border: '0.5px solid #e8e8e8', borderRadius: 12, padding: '14px 16px', marginBottom: 20, display: 'flex', gap: 10, alignItems: 'center' }}>
                  <select className="flex-1" value={selectedCatalogSectionId} onChange={(e) => setSelectedCatalogSectionId(e.target.value)}
                    style={{ ...inputStyle, flex: 1 }}>
                    <option value="">— Pick a catalog section —</option>
                    {catalogSections.map((s) => <option key={s.id} value={s.id}>{s.name}</option>)}
                  </select>
                  <button onClick={addTemplateSection} disabled={!selectedCatalogSectionId}
                  ...
                  <button onClick={() => { setShowAddSection(false); setSelectedCatalogSectionId('') }}
                    style={{ fontSize: 13, color: '#aaa', background: 'none', border: 'none', cursor: 'pointer' }}>Cancel</button>
                </div>
              )}

              {templateSections.length === 0 && <p style={{ color: '#bbb', fontSize: 13, marginBottom: 20 }}>No sections yet. Add one above.</p>}
                          ...
                          {addingItemsToSectionId === section.id ? (
                            <div style={{ marginBottom: 16 }}>
                              <div style={{ display: 'flex', gap: 10, alignItems: 'center', marginBottom: 10 }}>
                                <select value={addingItemsCatalogSectionId || ''} onChange={(e) => { setAddingItemsCatalogSectionId(e.target.value); if (e.target.value) getCatalogItemsForSection(e.target.value) }}
                                  style={{ ...inputStyle, flex: 1 }}>
                                  <option value="">— Filter by catalog section —</option>
                                  {catalogSections.map((s) => <option key={s.id} value={s.id}>{s.name}</option>)}
                                </select>
                                <button onClick={() => { setAddingItemsToSectionId(null); setAddingItemsCatalogSectionId(null); setCatalogItemsForSection([]) }}
                                  style={{ fontSize: 13, color: '#aaa', background: 'none', border: 'none', cursor: 'pointer', whiteSpace: 'nowrap' }}>Done</button>
                              </div>
                            ...
                            <button onClick={() => { setAddingItemsToSectionId(section.id); setAddingItemsCatalogSectionId(null); setCatalogItemsForSection([]) }}
                                        ...
                                        {costRows.length === 0 ? (
                                          ...
                                          <>
                                            ...
                                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 80px 100px', gap: 8, marginBottom: 4, padding: '0 2px' }}>
                                              {['Name', 'Unit', 'Quantity'].map((h) => <span key={h} style={{ fontSize: 11, color: '#bbb' }}>{h}</span>)}
                                            </div>
              ...
              <div style={{ background: 'white', border: '0.5px solid #e8e8e8', borderRadius: 14, padding: '16px 20px' }}>
                ...
                <textarea rows={20} placeholder="Paste your terms and conditions here..." value={tcText} onChange={(e) => setTcText(e.target.value)}
                  style={{ ...inputStyle, width: '100%', boxSizing: 'border-box', fontFamily: 'monospace', resize: 'none', lineHeight: 1.6, padding: '10px 12px' }} />
              </div>
            </>
          )}
        </div>
