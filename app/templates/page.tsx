...
export default function TemplatesPage() {
  ...
  async function deleteTemplateSection(sectionId: string) {
    ...
    const itemIds = (items || []).map((i: any) => i.id)
    if (itemIds.length > 0) { await supabase.from('template_item_rows').delete().in('item_id', itemIds); await supabase.from('template_items').delete().in('section_id', [sectionId]) }
  ...
  return (
    <div className="flex h-screen bg-[#f4f7fb]">
      ...
      {/* MAIN SIDEBAR */}
      <aside className="w-60 bg-white border-r border-gray-100 p-6 shadow-sm flex-shrink-0">
        ...
        <nav className="flex flex-col gap-3 text-sm">
          ...
          <Link href="/templates" className="text-blue-700 font-medium">Templates</Link>
        </nav>
      </aside>

      {/* TEMPLATES LIST SIDEBAR */}
      <div className="w-64 bg-white border-r p-6 flex flex-col flex-shrink-0">
        <h2 className="text-lg font-semibold mb-4">Templates</h2>

        <div className="mb-4 space-y-2">
          <input className="w-full p-2 border rounded text-sm" placeholder="Template name" value={newTemplateName} onChange={(e) => setNewTemplateName(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && createTemplate()} />
          <input className="w-full p-2 border rounded text-sm" placeholder="Description (optional)" value={newTemplateDescription} onChange={(e) => setNewTemplateDescription(e.target.value)} />
          <button onClick={createTemplate} className="w-full bg-blue-600 text-white py-2 rounded text-sm">+ New Template</button>
        </div>
        ...
        <div className="space-y-1 overflow-y-auto">
          {templates.map((t) => (
            <div
              key={t.id}
              className={`group flex items-center justify-between px-3 py-2 rounded-lg cursor-pointer text-sm ${selectedTemplateId === t.id ? 'bg-blue-100 text-blue-700 font-medium' : 'hover:bg-gray-100 text-gray-700'}`}
            ...
            >
              ...
              <button onClick={(e) => { e.stopPropagation(); deleteTemplate(t.id) }} className="ml-2 opacity-0 group-hover:opacity-100 text-xs text-gray-400 hover:text-red-500 transition-opacity">✕</button>
            </div>
          ))}
        </div>
      </div>
        ...
        {!selectedTemplateId ? (
          ...
          <>
            <div className="flex items-center justify-between mb-6">
              <h1 className="text-2xl font-semibold">{templates.find((t) => t.id === selectedTemplateId)?.name}</h1>
            </div>

            {/* Add section */}
            {!showAddSection ? (
              <button onClick={() => setShowAddSection(true)} className="mb-6 bg-blue-600 text-white px-4 py-2 rounded-lg text-sm">+ Add Section</button>
            ) : (
              <div className="bg-white border border-gray-200 rounded-xl p-4 mb-6 shadow-sm flex items-center gap-3">
                <select className="flex-1 border rounded-lg px-3 py-2 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-blue-300" value={selectedCatalogSectionId} onChange={(e) => setSelectedCatalogSectionId(e.target.value)}>
                  <option value="">— Pick a catalog section —</option>
                  {catalogSections.map((s) => (<option key={s.id} value={s.id}>{s.name}</option>))}
                </select>
                <button onClick={addTemplateSection} disabled={!selectedCatalogSectionId} className={`px-4 py-2 rounded-lg text-sm font-medium ${selectedCatalogSectionId ? 'bg-blue-600 text-white hover:bg-blue-700' : 'bg-gray-200 text-gray-400 cursor-not-allowed'}`}>Add</button>
                <button onClick={() => { setShowAddSection(false); setSelectedCatalogSectionId('') }} className="text-sm text-gray-400 hover:text-gray-600">Cancel</button>
              </div>
            )}

            {templateSections.length === 0 && <p className="text-gray-400 text-sm mb-6">No sections yet. Add one above.</p>}
                ...
                return (
                  <div key={section.id} className="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
                    <div className="flex items-center justify-between px-5 py-4">
                      <div className="flex-1 cursor-pointer" onClick={() => { if (isExpanded) { setExpandedSectionId(null) } else { setExpandedSectionId(section.id); getTemplateItems(section.id) } }}>
                        <p className="font-medium">{section.name}</p>
                        <p className="text-xs text-gray-400 mt-0.5">{items.length} item{items.length !== 1 ? 's' : ''}</p>
                      </div>
                      <div className="flex items-center gap-3">
                        <button onClick={() => deleteTemplateSection(section.id)} className="text-xs text-red-400 hover:text-red-600">Delete</button>
                        <span className="text-gray-400 text-sm cursor-pointer select-none" onClick={() => { if (isExpanded) { setExpandedSectionId(null) } else { setExpandedSectionId(section.id); getTemplateItems(section.id) } }}>{isExpanded ? '▲' : '▼'}</span>
                      </div>
                    </div>
                        ...
                        {addingItemsToSectionId === section.id ? (
                          <div className="mb-4">
                            <div className="flex items-center gap-3 mb-3">
                              <select className="border rounded-lg px-3 py-2 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-blue-300" value={addingItemsCatalogSectionId || ''} onChange={(e) => { setAddingItemsCatalogSectionId(e.target.value); if (e.target.value) getCatalogItemsForSection(e.target.value) }}>
                                <option value="">— Filter by catalog section —</option>
                                {catalogSections.map((s) => (<option key={s.id} value={s.id}>{s.name}</option>))}
                              </select>
                              <button onClick={() => { setAddingItemsToSectionId(null); setAddingItemsCatalogSectionId(null); setCatalogItemsForSection([]) }} className="text-sm text-gray-400 hover:text-gray-600 whitespace-nowrap">Done</button>
                            </div>
                            {catalogItemsForSection.length > 0 && (
                              <div className="space-y-1 max-h-48 overflow-y-auto border rounded-lg p-2 bg-gray-50">
                                {catalogItemsForSection.map((ci) => {
                                  ...
                                  return (
                                    <div key={ci.id} className="flex items-center justify-between px-3 py-2 rounded-lg bg-white border border-gray-100">
                                      <p className="text-sm text-gray-700">{ci.name}</p>
                                      {alreadyAdded ? <span className="text-xs text-green-500">Added</span> : <button onClick={() => addTemplateItem(ci, section.id)} className="text-xs text-blue-600 hover:text-blue-800 font-medium">+ Add</button>}
                                    </div>
                                  )
                                })}
                              </div>
                          ...
                          <button onClick={() => { setAddingItemsToSectionId(section.id); setAddingItemsCatalogSectionId(null); setCatalogItemsForSection([]) }} className="mb-4 text-sm text-blue-600 hover:text-blue-800 font-medium">+ Add Item from Catalog</button>
                        )}
                              ...
                              return (
                                <div key={item.id} className="border border-gray-100 rounded-xl overflow-hidden">
                                  <div className="flex items-center justify-between px-4 py-3 bg-gray-50">
                                    <div className="flex-1 cursor-pointer" onClick={() => { if (isItemExpanded) { setExpandedItemId(null) } else { setExpandedItemId(item.id); getTemplateItemRows(item.id, item.catalog_item_id) } }}>
                                      <p className="text-sm font-medium">{catalogItem?.name}</p>
                                      <p className="text-xs text-gray-400">{catalogItem?.description}</p>
                                    </div>
                                    <div className="flex items-center gap-3">
                                      <button onClick={() => deleteTemplateItem(item.id, section.id)} className="text-xs text-red-400 hover:text-red-600">Remove</button>
                                      <span className="text-gray-400 text-sm cursor-pointer select-none" onClick={() => { if (isItemExpanded) { setExpandedItemId(null) } else { setExpandedItemId(item.id); getTemplateItemRows(item.id, item.catalog_item_id) } }}>{isItemExpanded ? '▲' : '▼'}</span>
                                    </div>
                                  </div>
                                    ...
                                    <div className="px-4 py-3 border-t border-gray-100">
                                      <div className="grid grid-cols-2 gap-3 mb-4">
                                        <div>
                                          ...
                                          <input type="number" className="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-300" defaultValue={item.display_quantity || ''} onBlur={(e) => updateDisplayQuantity(item.id, section.id, e.target.value)} placeholder="e.g. 300" />
                                        </div>
                                        <div>
                                          <label className="text-xs text-gray-400 block mb-1">Display unit</label>
                                          <input className="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-300" defaultValue={item.display_unit || ''} onBlur={(e) => updateDisplayUnit(item.id, section.id, e.target.value)} placeholder="e.g. sq ft" />
                                        </div>
                                      </div>
                                            ...
                                            {costRows.map((cr: any) => {
                                              ...
                                              return (
                                                <div key={cr.id} className="grid grid-cols-3 gap-2 items-center">
                                                  ...
                                                  <input type="number" className="border rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-300" defaultValue={tir?.quantity || ''} onBlur={(e) => upsertTemplateItemRow(item.id, cr.id, Number(e.target.value))} placeholder="0" />
                                                </div>
                                              )
                                            })}
                                          </div>
            ...
            <div className="bg-white border border-gray-200 rounded-xl shadow-sm p-5">
              <div className="flex items-center justify-between mb-3">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <button onClick={saveTc} disabled={tcSaving} className="bg-blue-600 text-white px-4 py-1.5 rounded-lg text-sm hover:bg-blue-700 disabled:bg-gray-200 disabled:text-gray-400">
                    {tcSaving ? 'Saving…' : 'Save T&C'}
                  </button>
                </div>
              </div>
              <p className="text-xs text-gray-400 mb-2">This text will be included in the proposal preview and sent to the client for signature.</p>
              <textarea className="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-300 resize-none font-mono" rows={20} placeholder="Paste your terms and conditions here..." value={tcText} onChange={(e) => setTcText(e.target.value)} />
            </div>
          </>
        )}
      </div>
